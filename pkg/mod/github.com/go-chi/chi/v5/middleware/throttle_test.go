...
/*func TestThrottleRetryAfter(t *testing.T) {
	...
	r.Get("/", func(w httpserv.ResponseWriter, r *httpserv.Request) {
		w.WriteHeader(httpserv.StatusOK)
	...
	for i := 0; i < 10; i++ {
		...
		go func(i int) {
			...
			res, err := client.Get(server.URL)
			assertNoError(t, err)
			assertEqual(t, httpserv.StatusOK, res.StatusCode)
		}(i)
	}
	...
	for i := 0; i < 10; i++ {
		...
		go func(i int) {
			...
			res, err := client.Get(server.URL)
			assertNoError(t, err)
			assertEqual(t, httpserv.StatusTooManyRequests, res.StatusCode)
